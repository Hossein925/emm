...
const AddSectionModal: React.FC<AddSectionModalProps> = ({ isOpen, onClose }) => {
  const { addSection } = useAppContext();
  const [name, setName] = useState('');
  const [icon, setIcon] = useState('');
  const [colorClass, setColorClass] = useState('sky');

  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (name.trim() && icon.trim()) {
      await addSection(name, icon, colorClass);
      setName('');
      setIcon('');
      setColorClass('sky');
      onClose();
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 z-50 flex justify-center items-center p-4 transition-opacity duration-300" onClick={handleClose}>
      <div className="bg-white/80 backdrop-blur-xl border border-slate-200 rounded-2xl shadow-2xl p-8 w-full max-w-md relative animate-fade-in-up" onClick={e => e.stopPropagation()}>
        ...
        <h2 className="text-2xl font-bold text-center text-slate-700 mb-6">افزودن بخش جدید</h2>
